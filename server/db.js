...
const db = {
  ...
  // Comments
  getComments:(postId)=>T.comments.findAll(c=>c.post_id===+postId).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)).map(c=>{const u=T.users.findOne(x=>x.id===c.user_id);return{...c,username:u?.username,handle:u?.handle||u?.username,avatar:u?.avatar,gradient:u?.gradient,rank:u?.rank,verified:u?.verified||false,time:timeAgo(c.created_at),likes:c.likes||0,aiHighlighted:c.user_id===999};}),
  addComment:(data)=>{const c=T.comments.insert(data);if(!data.parent_id)T.posts.update(p=>p.id===+data.post_id,p=>{return{...p,comments_count:(p.comments_count||0)+1};});const u=T.users.findOne(x=>x.id===c.user_id);return{...c,username:u?.username,handle:u?.handle||u?.username,avatar:u?.avatar,gradient:u?.gradient,rank:u?.rank,verified:u?.verified||false,time:'just now',likes:0,aiHighlighted:c.user_id===999};},
