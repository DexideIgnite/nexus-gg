...
const db = {
  ...
  // Comments
  getComments:(postId)=>T.comments.findAll(c=>c.post_id===+postId).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)).map(c=>{const u=T.users.findOne(x=>x.id===c.user_id);return{...c,username:u?.username,handle:u?.handle||u?.username,avatar:u?.avatar,gradient:u?.gradient,rank:u?.rank,verified:u?.verified||false,badge_type:u?.badge_type||'',plan:u?.plan||'free',time:timeAgo(c.created_at),likes:c.likes||0,aiHighlighted:c.user_id===999};}),
  addComment:(data)=>{const c=T.comments.insert(data);if(!data.parent_id){const post=T.posts.findOne(p=>p.id===+data.post_id);if(post)T.posts.update(p=>p.id===+data.post_id,{comments_count:(post.comments_count||0)+1});}const u=T.users.findOne(x=>x.id===c.user_id);return{...c,username:u?.username,handle:u?.handle||u?.username,avatar:u?.avatar,gradient:u?.gradient,rank:u?.rank,verified:u?.verified||false,badge_type:u?.badge_type||'',plan:u?.plan||'free',time:'just now',likes:0,aiHighlighted:c.user_id===999};},
